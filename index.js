...
const ALERT_TONE_FILE = path.join(__dirname, 'assets', 'alert_tone.wav');
...
function evaluateTick(symbol, price) {
  ...
  for (const alert of alerts) {
    ...
    if (crossedUp || crossedDown) {
      ...
      // Telegram bots can't force a custom notification sound through the OS
      // - but they CAN attach a real audio file to a message. Sending the
      // actual alert tone here means you genuinely hear it, not just
      // Telegram's generic notification blip.
      bot.sendAudio(AUTHORIZED_CHAT_ID, ALERT_TONE_FILE, {
        title: 'Whisper Alert',
        performer: labelForSymbol(alert.symbol),
      }).catch((err) => console.error('[bot] failed to send alert tone:', err.message));
    } else {
      alert.last_price = price;
      changed = true;
    }
